...
5. Uninstall skills with `debugging-engine --uninstall <name>` (template-identical files only; `--force` deletes modified skill trees). Uninstall the uv tool with `debugging-engine uninstall-cli` (`uv tool uninstall debugging-engine`).
6. `--agent` and `--uninstall` accept a comma-separated list or `all` for batch install/uninstall in one call.
...
- Batch `--uninstall claude,cursor` / `--uninstall all` removes each skill root independently; `integration.json` still records a single last-installed agent.
